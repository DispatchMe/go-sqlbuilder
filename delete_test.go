...
func ExampleDelete_complex() {
	sql, vars := Delete().From("posts").LeftJoin("comments", OnColumn("comments.post_id", "posts.id")).GroupBy("posts.id").Having(LessThan{"COUNT(comments)", 10}).GetSQL()
...
func TestDelete(t *testing.T) {
	Convey("delete", t, func() {
		query, vars := Delete().From("foos").Where(Equal{"id", 10}).Limit(1).GetSQL()
